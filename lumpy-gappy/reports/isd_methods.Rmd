...
sims_ids <- lapply(sims_ids, add_all_pit_boundaries, threshold = 0.05)

sims_isd_method_summaries <- lapply(sims_ids, isd_methods_summary, threshold = 0.05) %>%
  dplyr::bind_rows(.id = "sim")

sims_plots <- lapply(sims_ids, plot_integrated_density, pit_boundaries = T)
